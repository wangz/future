...
query = "insert into future.future_data_all(origin,contract,company,value_type,real_value,pub_date) values (%s,%s,%s,%s,%s,%s)"
url_date = "20130711"
if sys.argv[1]!=None:
    url_date = sys.argv[1]
else:
...
##                time.sleep(0.1)
                if len(tr_values) > 0:
                    try:
                        conn = None
                        cursor = None
                        conn = MySQLdb.Connection('127.0.0.1', 'root', '', 'future',charset='utf8')
                        cursor = conn.cursor()
                        if tr_values[1]!='blank':
                            logging.info(query % ('上期',hydm,tr_values[1],'成交量',tr_values[2],url_date))
                            cursor.execute(query,('上期',hydm,tr_values[1],'成交量',tr_values[2],url_date))
                        if tr_values[5]!='blank':
                            logging.info(query % ('上期',hydm,tr_values[5],'持买单量',tr_values[6],url_date))
                            cursor.execute(query,('上期',hydm,tr_values[5],'持买单量',tr_values[6],url_date))
                        if tr_values[9]!='blank':
                            logging.info(query % ('上期',hydm,tr_values[9],'持卖单量',tr_values[10],url_date))
                            cursor.execute(query,('上期',hydm,tr_values[9],'持卖单量',tr_values[10],url_date))
                    except Exception,e:
